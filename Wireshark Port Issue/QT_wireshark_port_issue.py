import Assets.Custom_Modules.QT_custom_module as cms
import pyshark
import time
...
trace_path = ".\\Assets\\Traces\\Trace1.pcap"
...
# Existing DNS port lists (Editable)
dns_tcp_port_list = cms.dns_tcp_port_extractor()
dns_udp_port_list = cms.dns_udp_port_extractor()
...
if "diameter" == options_dict.get(user_option):
    ...
    print("Please close all wireshark instances and reopen.\n")
elif "dns" == options_dict.get(user_option):
    dns_tcp_list = []
    dns_udp_list = []
    capture = pyshark.FileCapture(trace_path)
    for packet in capture:
        if "DNS" in str(packet.layers):
            if "TCP" in str(packet.layers):
                dns_tcp_list.append(int(packet.tcp.get("tcp.srcport")))
                dns_tcp_list.append(int(packet.tcp.get("tcp.dstport")))
            if "UDP" in str(packet.layers):
                dns_udp_list.append(int(packet.udp.get("udp.srcport")))
                dns_udp_list.append(int(packet.udp.get("udp.dstport")))
    
    temp_dns_tcp_list = dns_tcp_list + dns_tcp_port_list
    temp_dns_udp_list = dns_udp_list + dns_udp_port_list

    print(f'DNS TCP = {cms.duplicate_removal(temp_dns_tcp_list)}\n')
    print(f'DNS UDP = {cms.duplicate_removal(temp_dns_udp_list)}\n')

    print(cms.dns_tcp_port_writer(temp_dns_tcp_list))
    print(cms.dns_udp_port_writer(temp_dns_udp_list))
